	...
	for ( ; ; ) {
		...
		if(fork() == 0){
				...
				{	
					...
					if( (unready = select(maxfdp1, &urset, NULL,NULL,NULL))<0){
				            if(errno == EINTR)
               					 continue;
            				    else
				                perror("select error");
					...
					if(FD_ISSET(udpsockfd2, &urset)){
						...
						recvfrom(udpsockfd2, udpbuff2, sizeof(udpbuff2), 0, (struct sockaddr*) &udpcliaddr2, (socklen_t*) &udpcli_len2);
					...
					if(FD_ISSET(udpsockfd3, &urset)){
						while(1){
							...
							if(udpbuff3[strlen(udpbuff3)-1] == '\n') udpbuff3[strlen(udpbuff3)-1] = '\0';
							if(strcmp(udpbuff3, "quit")==0){
								bzero(udpbuff3, sizeof(udpbuff3));
								strcpy(udpbuff3, "quit");
								sendto(udpsockfd3, (char *) udpbuff3, strlen(udpbuff3), 0, (const struct sockaddr *) &udpcliaddr3, sizeof(udpcliaddr3));
								break;
							}
							bzero(udpbuff3, sizeof(udpbuff3));
							strcpy(udpbuff3, "continue Chatting");
							sendto(udpsockfd3, (char *) udpbuff3, strlen(udpbuff3), 0, (const struct sockaddr *) &udpcliaddr3, sizeof(udpcliaddr3));
							printf("CHATTING SERVER send : %s \n", udpbuff3);
						}					
